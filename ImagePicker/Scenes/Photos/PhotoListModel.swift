...
protocol PhotoListModelType: class {
    func fetchPhotos(options: PhotoFetchOptions, closure: @escaping (Result<PhotoSection, Error>) -> Void)
}

final class PhotoListModel: PhotoListModelType {
    func fetchPhotos(options: PhotoFetchOptions, closure: @escaping (Result<PhotoSection, Error>) -> Void) {
